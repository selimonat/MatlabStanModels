...
%% many subjects different modalities;
S       = 20;
T       = 5;
x       = deg2rad([-135:45:180]');
offset  = linspace(1,5,T)';
amp     = linspace(1,5,T)';
kappa   = logspace(-1.4,1.2,T)';
...
%         subject  = [subject si];
        %y(ti,:) = [offset(ti) + amp(ti)*exp(-(x/sd(ti)).^2) + randn(8,1)*sigma_y(ti)]';
        y = [y VonMises(x,amp(ti),kappa(ti),0,offset(ti)) + randn(8,1)*sigma_y(ti)];    
    end
end
imagesc(y);
% ok estimate initial values with MLE
...
d.y              = data.y';
d.x              = repmat(data.x(:)',size(d.y,1),1);
d.ids            = 1:data.T;
t2               = Tuning(d);
t2.visualization = 1;
t2.gridsize      = 10;
...
t2.SingleSubjectFit(nfun);
init.amp         = t2.fit_results{nfun}.params(:,1);
init.kappa       = max(2,t2.fit_results{nfun}.params(:,2));
init.offset      = min(15,t2.fit_results{nfun}.params(:,3));
init.sigma_y     = repmat(mean(t2.fit_results{nfun}.params(:,4)),1,5);
init.mu_amp      = repmat(mean(init.amp),1,5);
init.mu_offset   = repmat(mean(init.offset),1,5);
init.mu_kappa    = repmat(median(init.kappa),1,5);
init.sigma_amp   = repmat(0.5,1,5)
init.sigma_offset=repmat(0.5,1,5);
init.sigma_kappa =repmat(0.5,1,5);
