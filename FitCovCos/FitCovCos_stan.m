function [fit]=FitCovCos_stan(z,varargin)
...
y     = Y(:);
z     = z(:);
data  = struct('x',x,'y',y,'z',z,'N',size(x,1),'D',size(x,2),'y_new',y,'x_new',x,'N_new',size(x,1));
!rm output-* FitCovCos FitCovCos.cpp
fit = stan('file','FitCovCos.stan','data',data,'verbose',true,'iter',400,varargin{:});
fit.block();
%%
print(fit);
%% plot the parameters
...
%
subplot(2,3,2)
MakeHist('std',fit.extract.freq(:,1));

subplot(2,3,3)
plot(fit.extract.freq,fit.extract.amp,'o')
